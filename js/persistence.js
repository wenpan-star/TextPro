// filename: js/persistence.js
/**
 * ============================================================================
 * persistence.js — 自动保存调度 + 用户配置持久化
 * ============================================================================
 *
 * 【本次修订说明 — 修复"用户主动跳过"误覆盖磁盘的 P0 缺陷】
 *
 *   一、【P0 修复】分支 1（用户主动关闭"启动时恢复规则与设置"）
 *     · 原实现：
 *         initializeDefaultState();          // 重置 AppState 为默认值
 *         AppState.autoLoadSnapshot = false;
 *         cachedCryptoKey = await getOrCreateCryptoKey();
 *         AppState.databaseReady = true;     // 允许后续自动保存
 *         return { loaded: false, reason: 'skipped' };
 *     · 后果：用户只要取消勾选这个复选框一次，下次刷新：
 *         1. 走分支 1 → AppState 重置为空
 *         2. databaseReady = true
 *         3. main.js 调度 scheduleAutoSave() → 500ms 后写盘
 *         4. 空数据覆盖了用户原有数据
 *         5. 用户重新勾选后刷新，看到的是被覆盖过的空数据
 *     · 现实现：
 *         AppState.userSkippedAutoLoad = true;   // 标记本次会话跳过
 *         AppState.autoLoadSnapshot = false;
 *         AppState.databaseReady = false;        // 禁止自动保存
 *         AppState.cryptoKeyReady = false;       // 不初始化密钥
 *         return { loaded: false, reason: 'skipped' };
 *     · 保证"用户主动跳过 → 本次会话既不加载也不保存"，保护磁盘数据。
 *
 *   二、其余逻辑保持不变
 * ============================================================================
 */
...
function estimateCurrentTextByteLengthUpperBound() {
    ...
    if (AppState.sourceTextStale && DOM.sourceTextarea) {
        AppState.sourceText = DOM.sourceTextarea.value;
        AppState.sourceTextStale = false;
        sourceLength = AppState.sourceText.length;
    } else {
        sourceLength = AppState.sourceText ? AppState.sourceText.length : 0;
    }

    if (AppState.resultTextStale && DOM.resultTextarea) {
        AppState.resultText = DOM.resultTextarea.value;
        AppState.resultTextStale = false;
        resultLength = AppState.resultText.length;
    } else {
        resultLength = AppState.resultText ? AppState.resultText.length : 0;
    }
...
export async function loadStateOnStartup() {
    ...
    //
    // ★ P0 修复：不再调用 initializeDefaultState()，不初始化密钥，
    //   禁止自动保存。避免"重置为空后覆盖磁盘"。
    if (mirroredAutoLoadSnapshot === false) {
        AppState.userSkippedAutoLoad = true;

        // 仍然需要一份基本的默认值供本会话 UI 渲染
