 ...
 *
 *   一、【P1 修复】过滤缓存失效时机
 *     · 原实现：isPresetFilterCacheValid 只比对"查询词 + 字段范围 + 数据源
 *       + AppState.presets 引用"四元组。用户编辑预设内规则内容时，
 *       AppState.presets 引用不变，缓存命中，左侧列表 matchingRuleCount
 *       徽章显示陈旧匹配数。
 *     · 现实现：touchSelectedPreset 内新增 clearPresetFilterCache()，
 *       任何预设内容变更都会失效过滤缓存。
 *
 *   二、【P2 修复】renderPresetList 的 scrollTop 恢复冲突
 *     · 原实现：renderPresetList 内部用 rAF 恢复 container.scrollTop，
 *       openPresetManagerModal 又在同步赋值 AppState.presetListScrollTop，
 *       两者可能互相覆盖导致视觉闪烁。
 *     · 现实现：renderPresetList 新增可选参数 preferredScrollTop；
 *       openPresetManagerModal 传入 AppState.presetListScrollTop，
 *       删除末尾的同步赋值，仅由内部 rAF 统一恢复。
 *
 *   三、【P1 修复】loadPresetToMain 完成后重置脏标志
 *     · 加载预设后，主表内容 == 预设内容，视为已同步。
 *       重置 mainTableDirtyForActivePreset = false，
 *       避免 beforeunload 误报"未保存修改"。
 *
 *   四、【P2 修复】导出 checkMainTableHasUnsavedChanges
 *     · 供 ui.js 的 beforeunload 处理器使用统一的权威判断。
 *
 *   五、【P3 清理】移除 syncPresetRuleEditToCustomRules 未使用的 oldHash 参数
 *
 *   六、保留全部既有功能
 * ============================================================================
 */
...
/**
 * 检查当前工作区是否存在"相对于其对应预设"的未保存修改。
 *
 * 语义：
 *   · 当前工作区是默认规则 → 比对 customRules 与 defaultRules
 *   · 当前工作区是命名预设 → 比对 customRules 与 activePreset.rules
 *   · 当前工作区指向的预设不存在 → 视为有未保存修改（保守）
 *
 * ★ 导出原因：
 *   ui.js 的 beforeunload 处理器需要与主表 UI 使用同一份权威判断。
 *   原实现用 mainTableDirtyForActivePreset 标志位，存在两个问题：
 *     1. 加载预设后未重置，切到新预设仍保留脏标记
 *     2. 与"实际内容是否一致"解耦，标志位可能与实际内容不符
 *
 * @returns {boolean} true 表示存在未保存的修改
 */
...
export function openPresetManagerModal() {
    ...
    // ★ 修复：把上次关闭时的滚动位置作为 preferredScrollTop 传给
    //   renderPresetList，由它内部的 rAF 统一恢复；
    //   原先在渲染完成后同步赋值 scrollTop 会与内部 rAF 冲突。
...
function shouldRenderMainTableVirtualItem(normalizedQuery) {
    ...
    // ★ 精确别名匹配：只有查询词是别名的前缀（或别名是查询词的前缀）才匹配
    //   避免 'in' 匹配 'main'
 ...
 * 渲染预设列表。
 *
 * @param {number} [preferredScrollTop]
 *   可选的"优先恢复"的 scrollTop。当调用方希望恢复一个特定位置（例如
 *   openPresetManagerModal 恢复上次关闭时的位置）时传入。
 *   不传时使用当前 container.scrollTop（保持可视位置不变）。
 */
function renderPresetList(preferredScrollTop) {
    ...
    // ★ 修复：优先使用调用方传入的 preferredScrollTop，否则保持当前 scrollTop。
    //   这样 openPresetManagerModal 能恢复上次位置，其它场景保持可视位置不变。
    ...
    // ★ rAF 恢复 scrollTop，避免 replaceChildren 后布局未完成时被钳制
    if (savedScrollTop > 0) {
...
function renderPresetRulesList() {
    ...
    const normalizedQuery = searchQuery.trim().toLowerCase();
    let displayedRules = preset.rules;
    ...
    if (isFilteredView) {
        displayedRules = preset.rules.filter(function filterRule(rule) {
            const ruleName = String(rule.name || '').toLowerCase();
            const rulePattern = String(rule.pattern || '').toLowerCase();
            const ruleReplacement = String(
                rule.replacement || ''
        ...
        if (displayedRules.length < preset.rules.length) {
    ...
    const visibleRulesReference = isFilteredView ? displayedRules : null;

    displayedRules.forEach(function forEachRule(rule) {
        const card = createPresetRuleCard(
            rule,
            preset,
            normalizedQuery,
            visibleRulesReference
        );
...
function createPresetRuleCard(
    ...
    moveUpButton.onclick = function onMoveUp() {
        movePresetRuleRelativeToVisible(rule, -1, visibleRulesReference);
    };
    ...
    replacementInput.addEventListener('change', function onReplaceChange(event) {
        ...
        // ★ 若替换文本以 @js: 开头，会触发 confirmJSMode，传 rule.id
    ...
    if (existenceLocations.length > 0) {
        ...
        card.appendChild(existenceRow);
    }

    // ============ 搜索高亮 ============
    const shouldHighlightRuleContent = (
        searchQueryForHighlight &&
        searchQueryForHighlight !== '' &&
        AppState.presetSearchScope === 'all'
    );

    if (shouldHighlightRuleContent) {
        if (
            rule.name &&
            String(rule.name).toLowerCase().indexOf(
                searchQueryForHighlight
            ) !== -1
        ) {
            nameInput.classList.add('preset-input-search-hit');
        }
        if (
            rule.pattern &&
            String(rule.pattern).toLowerCase().indexOf(
                searchQueryForHighlight
            ) !== -1
        ) {
            patternInput.classList.add('preset-input-search-hit');
        }
        if (
            rule.replacement &&
            String(rule.replacement).toLowerCase().indexOf(
                searchQueryForHighlight
            ) !== -1
        ) {
            replacementInput.classList.add('preset-input-search-hit');
        }
    }
 ...
 * 规则：
 *   · filterQuery 为空 → 全部匹配
 *   · 主表显示名：包含 filterQuery 即匹配（宽松，因为显示名是用户自定义的）
 *   · 别名（主表 / 默认规则 / main / default）：
 *     filterQuery 是别名的前缀，或别名是 filterQuery 的前缀
 *     这样 'in' 不会匹配 'main'（'in' 不是 'main' 的前缀，'main' 也不是 'in' 的前缀）
 */
...
function movePresetRuleRelativeToVisible(rule, direction, visibleRulesReference) {
    const preset = getSelectedPreset();
    if (!preset || !Array.isArray(preset.rules)) return;

    const isVirtualMain = preset.isVirtual === true;

    if (visibleRulesReference && Array.isArray(visibleRulesReference)) {
...
function deletePreset() {
    ...
    if (wasActive) {
        ...
        // ★ 切回默认规则：主表内容 == defaultRules，视为已同步
